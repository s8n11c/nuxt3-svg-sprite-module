  ...
  async setup(options, nuxt) {
    ...
    nuxt.hook("nitro:init", async (nitro) => {
      if (options.input.startsWith("/")) {
        // using fsp instead of nitro because nitro doesn't support external access
        ...
        const svgsFiles = await fsp.readdir(input);
        ...
        await Promise.all(
          svgsFiles.map(async (file) => {
            ...
            return addSvg({
              name,
              sprite,
              content: await fsp.readFile(`${input}/${file}`, "utf-8"),
            });
          })
        );

        const writeSprite = async (sprite: any) => {
          await fsp.writeFile(
